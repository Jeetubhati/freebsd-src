...
{
	char cwd[MAXPATHLEN], abspath[MAXPATHLEN * 2];
	char *paths[2];
	FTS *fts;
	FTSENT *ent;
	int entries;
	...
	snprintf(abspath, sizeof(abspath), "%s/dir", cwd);
	paths[0] = abspath;
	paths[1] = NULL;
	...
	while ((ent = fts_read(fts)) != NULL) {
		/*
		 * Chdir to root and back between every fts_read() call.
		 * With FTS_NOCHDIR this must not affect the traversal.
		 */
		ATF_REQUIRE_EQ(0, chdir("/"));
		ATF_REQUIRE_EQ(0, chdir(cwd));
		entries++;
	}
...
ATF_TC_BODY(name_nul_terminated, tc)
{
	...
	while ((ent = fts_read(fts)) != NULL) {
		...
		ATF_CHECK_EQ_MSG('\0', ent->fts_name[ent->fts_namelen],
		    "no NUL at fts_name[fts_namelen] for '%s'",
		    ent->fts_name);
	}
...
ATF_TC_BODY(ftsent_fields, tc)
{
	...
	while ((ent = fts_read(fts)) != NULL) {
		...
		if (ent->fts_info == FTS_D) {
			...
			ATF_CHECK_MSG(ent->fts_nlink >= 2,
			    "directory fts_nlink < 2 for '%s'",
			    ent->fts_name);
		}
	}
...
ATF_TC_BODY(cycle_detection, tc)
{
	...
	while ((ent = fts_read(fts)) != NULL) {
		if (ent->fts_info == FTS_DC)
			saw_dc = 1;
		ATF_REQUIRE_MSG(++entries < 200,
		    "traversal exceeded 200 entries, probable infinite loop");
	}
...
ATF_TC_BODY(close_after_root_deleted, tc)
{
	...
	/* Drain traversal -- errors expected, no crash. */
...
{
	char orig_cwd[MAXPATHLEN], final_cwd[MAXPATHLEN];
	char *paths[] = { "dir", NULL };
	FTS *fts;

	ATF_REQUIRE(getcwd(orig_cwd, sizeof(orig_cwd)) != NULL);
	ATF_REQUIRE_EQ(0, mkdir("dir", 0755));
	ATF_REQUIRE_EQ(0, close(creat("dir/file", 0644)));
	...
	ATF_REQUIRE(getcwd(final_cwd, sizeof(final_cwd)) != NULL);
	ATF_CHECK_STREQ_MSG(orig_cwd, final_cwd,
	    "CWD after fts_close should be '%s', got '%s'",
	    orig_cwd, final_cwd);

	ATF_REQUIRE_EQ(0, unlink("dir_moved/file"));
	ATF_REQUIRE_EQ(0, rmdir("dir_moved"));
}
 ...
 * Requires a nullfs bind-mount to test properly.
 */
ATF_TC(xdev);
ATF_TC_HEAD(xdev, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "FTS_XDEV does not cross mount points (requires nullfs)");
	atf_tc_set_md_var(tc, "require.user", "root");
}
ATF_TC_BODY(xdev, tc)
{
	atf_tc_skip("requires nullfs bind-mount setup");
}

/*
 * FTS_WHITEOUT returns whiteout objects from union mounts.
 * Requires a union mount to test properly.
 */
ATF_TC(whiteout);
ATF_TC_HEAD(whiteout, tc)
{
	atf_tc_set_md_var(tc, "descr",
	    "FTS_WHITEOUT returns whiteout objects (requires union mount)");
	atf_tc_set_md_var(tc, "require.user", "root");
}
ATF_TC_BODY(whiteout, tc)
{
	atf_tc_skip("requires union mount setup");
}
