...
/*
 * fts_set with an invalid instruction must return non-zero with EINVAL.
...
ATF_TC_BODY(invalid_options, tc)
{
	...
	errno = 0;
	ret = fts_set(fts, ent, 99);
	ATF_CHECK_MSG(ret != 0,
	    "fts_set with bad instr must fail (return non-zero)");
	ATF_CHECK_EQ_MSG(EINVAL, errno,
	    "expected EINVAL from fts_set with bad instr, got %d", errno);
 ...
 * times total.  Each fts_read() clears fts_instr, so the caller must
